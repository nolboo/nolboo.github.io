...
> [cron 보다 이벤트 드리븐 파일 와치 Watchman](https://nolboo.kim/blog/2016/10/01/event-driven-file-watch-watchman/) 글에서 파일이 변경되면 자동으로 저장소로 푸시하도록 하는 방법을 설명했다. 이때 sync.sh는 입력을 받지 않고 항상 같은 커밋메시지로 커밋하도록 변경해야 한다.
